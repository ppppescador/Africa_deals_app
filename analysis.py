...
def load_and_clean(uploaded_file, password):
    ...
    bracket_map = {
        'n.a': np.nan, '$100K-$500K': 0.3, '$500K-$1M': 0.75,
        '$1M-$2M': 1.5, '$2M-$5M': 3.5, '$5M-$10M': 7.5,
        '$10M-$50M': 30.0, '$50M-$100M': 75.0, '$100M+': 150.0, 'Unknown': np.nan,
    }
    ...
    def simplify_type(t):
        t = str(t).strip()
        if any(x in t for x in ['Series D','Series E','Series F']): return 'Series D+'
    ...
    def identify_yc(val):
        ...
        return 1 if (v.startswith('YC') or 'Y COMBINATOR' in v) else 0
    ...
    df_deals['Founders_Num'] = (
        df_deals['# of Founders'].astype(str).str.extract(r'(\d+)')[0].astype(float)
    )
    df_deals['is_disclosed'] = df_deals['Amount_numeric'].notna().astype(int)

    def categorize_market(country):
        if country in ['Nigeria','Kenya','South Africa','Egypt']: return 'Big 4'
...
def compute_summary_stats(df_deals):
    return {
        ...
        'climate_pct':    df_deals['Climate Tech'].astype(str).str.lower().eq('climate tech').mean()*100,
        'female_ceo_pct': (df_deals['CEO Gender']=='Female').mean()*100,
        'yc_count':       int(df_deals['is_yc'].sum()),
...
def create_interactive_map(df_deals, df_inv_final):
    top_sector_dict = df_deals.groupby('Country')['Sector'].apply(
    ...
    def get_clean_popup_html(country_df):
        ...
        for i, (inv, count) in enumerate(top_5.items()):
            ...
            html += (f"<li style='background:{bg};padding:4px 8px;border-radius:4px;"
                     f"margin-bottom:2px;display:flex;justify-content:space-between;font-size:0.9em;'>"
                     f"<span style='font-weight:600;color:#2c3e50;'>{inv}</span>"
                     f"<span style='color:#27ae60;'>{int(count)}</span></li>")
        html += "</ul>"
        return html

    top_5_html_dict = df_clean_inv.groupby('Country').apply(
        get_clean_popup_html, include_groups=False
    ).to_dict()
    ...
    folium.Choropleth(
        geo_data=geo_data, data=stats_map,
        columns=['Country','Nb_Deals'], key_on='feature.properties.name',
        fill_color='YlGnBu', fill_opacity=0.8, line_opacity=0.3,
        nan_fill_color='#ecf0f1', legend_name='Market Activity (Number of Deals)', bins=8
    ).add_to(m)
    folium.GeoJson(
        geo_data,
...
def plot_deal_landscape(df_deals):
    ...
    sns.countplot(data=df_deals, y='Type',
                  ...
                  palette='mako', ax=axes[0,0], hue='Type', legend=False)
    ...
    sns.countplot(data=df_deals, x='Bracket',
                  ...
                  palette='rocket', ax=axes[0,1], hue='Bracket', legend=False)
    ...
    stats_text = (
        ...
        f"Disclosed amounts : {df_deals['Amount_numeric'].notna().sum():,}"
    )
    ...
    plt.tight_layout()
    return fig  # ← UNE seule figure


def plot_market_trajectory(df_deals):
    """Retourne DEUX figures : annuelle et mensuelle."""
    ...
    ax2 = ax1.twinx()
    ax1.bar(x_pos, evolution['Count'], color='#3498db', alpha=0.5, label='Number of deals')
    ...
    fig1.suptitle('African Market Trajectory — Annual Overview', fontsize=15)
    plt.tight_layout()

    # ── Figure 2 : mensuelle (18 derniers mois) ──
    df_deals2 = df_deals.copy()
    df_deals2['YearMonth'] = pd.to_datetime(df_deals2['Deal Date']).dt.to_period('M')
    monthly = (df_deals2.groupby('YearMonth')
               .agg(Count=('Start-up name','count'), Total_USD_M=('Amount_clean','sum'))
               .tail(18))
    ...
    ax3.bar(x, monthly['Count'], color='#3498db', alpha=0.6, label='Number of deals')
    ax3.set_xticks(list(x))
    ax3.set_xticklabels(monthly.index, rotation=45, ha='right')
    ax3.set_ylabel('Number of Deals')

    ax4.plot(x, monthly['Total_USD_M'], marker='o', color='#e74c3c',
             linewidth=2.5, label='Total Capital ($M)')
    ax4.set_ylabel('Total Capital ($M)')

    for i, label in enumerate(monthly.index):
        if label.startswith(current_year):
            ax3.axvspan(i - 0.5, i + 0.5, alpha=0.08, color='#f39c12')

    lines3, labels3 = ax3.get_legend_handles_labels()
    lines4, labels4 = ax4.get_legend_handles_labels()
    ax3.legend(lines3 + lines4, labels3 + labels4, loc='upper left')
    fig2.suptitle(f'Monthly Deal Activity — Last 18 Months ({current_year} highlighted)',
                  fontsize=15)
    plt.tight_layout()

    return fig1, fig2  # ← DEUX figures
...
def plot_heatmaps(df_deals):
    ...
    df_heat = df_deals[df_deals['Sector'].isin(top_sectors) & df_deals['Region'].isin(top_regions)]
    pivot_total  = df_heat.pivot_table(values='Amount_clean', index='Sector', columns='Region', aggfunc='sum').fillna(0)
    pivot_median = df_heat.pivot_table(values='Amount_clean', index='Sector', columns='Region', aggfunc='median').fillna(0)
...
def plot_transparency(df_deals):
    ...
    ax1.plot(climate_trend.index, climate_trend.values, marker='o', color='#27ae60', linewidth=3)
    ...
    ax1.set_ylabel('%')
    ax1.grid(alpha=0.3)
    ax2.plot(disclosure_rate.index, disclosure_rate.values, marker='o', color='#3498db', linewidth=3)
...
def plot_top_countries(df_deals):
    ...
    sns.barplot(x=top_vol['Nb_Deals'], y=top_vol.index, palette='mako', ax=ax1, hue=top_vol.index, legend=False)
    ax1.set_title('Top 10 Countries by Deal Count')
    sns.barplot(x=top_val['Total_USD_M'], y=top_val.index, palette='rocket', ax=ax2, hue=top_val.index, legend=False)
...
def plot_market_concentration(df_deals):
    ...
    ax1.pie(market_dist.values, labels=market_dist.index,
            autopct='%1.1f%%', colors=['#2c3e50','#e74c3c','#95a5a6'], startangle=140)
    ...
    ax2.plot(emerging_trend.index, emerging_trend.values, marker='o', color='#27ae60', linewidth=3)
    ax2.axhline(avg, color='gray', linestyle='--', alpha=0.6)
    ax2.text(emerging_trend.index.min() + 0.1, avg + 0.5, f'Average: {avg:.1f}%', fontsize=9, color='gray')
    ax2.set_title('Emerging Markets Share per Year (%)')
    ax2.set_ylabel('%')
    ax2.grid(alpha=0.3)
    ax2.set_ylim(0, min(emerging_trend.values.max() * 1.2, 100))
    plt.tight_layout()
    return fig
...
def plot_pan_african(df_deals):
    pan_african_comp = (
        df_deals
        .assign(Scope=lambda d: d['Country'].eq('Africa').map({True:'Pan-African', False:'Country-Specific'}))
        .groupby('Scope')
        .agg(Nb_Deals=('Start-up name','count'), Median_Amount=('Amount_clean','median'))
        .reset_index()
    )
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
    sns.barplot(x='Scope', y='Nb_Deals', data=pan_african_comp, palette='Paired', hue='Scope', legend=False, ax=ax1)
    ax1.set_title('Number of Deals: Pan-African vs Country-Specific')
    ax1.set_ylabel('Number of deals')
    sns.barplot(x='Scope', y='Median_Amount', data=pan_african_comp, palette='Paired', hue='Scope', legend=False, ax=ax2)
...
def plot_sector_trends(df_deals):
    ...
    area_perc.plot(kind='area', stacked=True, alpha=0.75, ax=ax, colormap='tab10')
...
def plot_deal_size_by_sector(df_deals):
    """Retourne une figure MATPLOTLIB (pas Plotly) pour compatibilité download_chart."""
    median_size = (df_deals.groupby('Sector')['Amount_clean']
                   .median().sort_values(ascending=False).head(10))
    fig, ax = plt.subplots(figsize=(12, 6))
    sns.barplot(x=median_size.values, y=median_size.index,
                hue=median_size.index, palette='viridis', legend=False, ax=ax)
    ax.set_title('Median Deal Size by Sector ($M)', fontsize=14)
...
def plot_sector_stage(df_deals):
    ...
    df_cross = df_deals[df_deals['Type_Simple'].isin(main_types) & df_deals['Sector'].isin(top_sectors)]
    pivot_corr = (df_cross.groupby(['Sector','Type_Simple']).size()
                  .unstack(fill_value=0).reindex(columns=main_types, fill_value=0))
...
def plot_funding_funnel(df_deals):
    ...
    conversion_text = ""
    for f, t in [('Seed','Series A'),('Series A','Series B'),('Series B','Series C')]:
        if f in funnel_data.index and t in funnel_data.index and funnel_data[f] > 0:
            conversion_text += f"{f}→{t}: {(funnel_data[t]/funnel_data[f])*100:.0f}%  "
    ...
    ax.set_title(f'Startup Funding Funnel\nMacro conversion rates: {conversion_text}', fontsize=13)
...
def plot_time_between_rounds(df_deals):
    ...
    for f, t in transitions:
        ...
        by_s = (sub.groupby('Sector')['Days_Between_Rounds']
                ...
                .query('N >= 3')
                .assign(Median_Months=lambda d: d['Median_Days']/30, Transition=f'{f} → {t}')
                .reset_index())
        results.append(by_s)
    ...
    sns.barplot(x=gap_by_sector['Median_Months'], y=gap_by_sector.index,
                hue=gap_by_sector.index, palette='coolwarm_r', legend=False, ax=ax_global)
    for i, (_, row) in enumerate(gap_by_sector.iterrows()):
        ax_global.text(row['Median_Months']+0.2, i, f"n={int(row['N_Rounds'])}", va='center', fontsize=9)
    ax_global.set_title('Median Interval Between Rounds by Sector — All Transitions', fontsize=13)

    palette = {'Seed → Series A':'#3498db','Series A → Series B':'#27ae60','Series B → Series C':'#e67e22'}
    for ax_pos, (f, t) in zip([gs[1,0], gs[1,1], gs[1,2]], transitions):
        ...
        if subset.empty:
            ax.axis('off')
            continue
        sns.barplot(x='Median_Months', y='Sector', data=subset, color=palette[label], ax=ax, legend=False)
        for i, (_, row) in enumerate(subset.iterrows()):
            ax.text(row['Median_Months']+0.3, i, f"n={int(row['N'])}", va='center', fontsize=9)
        ax.set_title(label, fontsize=12, fontweight='bold', color=palette[label])
        ax.set_xlabel('Median months')
        ax.grid(axis='x', alpha=0.3)
    plt.suptitle('Time Between Rounds — Global & By Stage Transition', fontsize=16, fontweight='bold')
    plt.tight_layout()
    return fig


def plot_first_time_fundraisers(df_deals):
    first_deals = df_deals.sort_values('Deal Date').groupby('Start-up name').first().reset_index()
    top_sectors   = first_deals['Sector'].value_counts().head(10)
    top_countries = first_deals['Country'].value_counts().head(10)
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 7))
    sns.barplot(x=top_sectors.values, y=top_sectors.index, palette='mako', hue=top_sectors.index, legend=False, ax=ax1)
    ax1.set_title('Key Sectors for First Fundraising Rounds')
    ax1.set_xlabel('Number of first deals')
    sns.barplot(x=top_countries.values, y=top_countries.index, palette='rocket', hue=top_countries.index, legend=False, ax=ax2)
...
def plot_non_dilutive(df_deals):
    non_equity_types = ['Debt','Grant','Bonds','Bond','Green Bonds','Revenue-based financing','Convertible Note']
    non_equity = df_deals[df_deals['Type'].str.contains('|'.join(non_equity_types), case=False, na=False)].copy()
    non_equity = non_equity[non_equity['Sector'].notna()]
    if non_equity.empty:
        fig, ax = plt.subplots()
        ax.text(0.5, 0.5, 'No non-dilutive deals found', ha='center', va='center')
        return fig
    fig, ax = plt.subplots(figsize=(12, 7))
...
def plot_survival_signals(df_deals):
    ...
    last_deal['Years_Since'] = (pd.Timestamp('today') - last_deal['Deal Date']).dt.days / 365
    last_deal['Status'] = pd.cut(last_deal['Years_Since'], bins=[0,1.5,3,100],
    ...
    ax1.set_title('Startup Activity Status', fontsize=13)

    last_deal_sector = df_deals.sort_values('Deal Date').groupby('Start-up name').last().reset_index()
    last_deal_sector['Years_Since'] = (pd.Timestamp('today') - last_deal_sector['Deal Date']).dt.days / 365
    last_deal_sector['Status'] = pd.cut(last_deal_sector['Years_Since'], bins=[0,1.5,3,100],
                                         labels=['Active (<18m)','Slowing (18m–3y)','Dormant (3y+)'])
    pivot = (last_deal_sector.groupby(['Sector','Status']).size()
             .unstack(fill_value=0).apply(lambda x: x/x.sum()*100, axis=1))
...
def plot_gender_diversity(df_deals):
    ...
    sns.barplot(x=reliable['pct'], y=reliable.index, palette='RdPu', hue=reliable.index, legend=False, ax=ax2)
    ax2.set_title('Female CEOs by Sector (>10 deals)')
    ax2.set_xlabel('%')
    plt.tight_layout()
    return fig
...
def plot_academic_background(df_deals):
    edu_continent = df_deals['CEO - University/School - Continent'].value_counts().dropna()
    ...
    sns.boxplot(data=df_edu, x='CEO - University/School - Continent', y='Amount_clean',
                palette='Set3', hue='CEO - University/School - Continent', legend=False, ax=ax2)
    ax2.set_yscale('log')
...
def plot_team_size(df_deals):
    df_t = df_deals.copy()
    df_t['Founders_Num'] = df_t['# of Founders'].astype(str).str.extract(r'(\d+)')[0].astype(float)
    df_t['Founders_Group'] = df_t['Founders_Num'].apply(
        lambda x: '1' if x==1 else '2' if x==2 else '3' if x==3 else '4+' if x>=4 else np.nan
    )
    ...
    sns.barplot(data=df_t, x='Founders_Group', y='Amount_clean', order=['1','2','3','4+'],
                estimator='mean', palette='Oranges', hue='Founders_Group', legend=False, ax=ax2)
...
def plot_top_investors(df_inv_final):
    ...
    sns.barplot(x=top15.values, y=top15.index, palette='viridis', hue=top15.index, legend=False, ax=ax)
...
def plot_investor_clustering(df_investisseurs, years_cols):
    ...
    df_investisseurs = df_investisseurs.copy()
    ...
    cluster_labels = {0:'Consistent Mid-Tier',1:'Occasional / One-Shot',
                      2:'Boom & Retreat (2021 peak)',3:'Dominant Late Surge'}
    fig, ax = plt.subplots(figsize=(12, 6))
...
def plot_capital_origins(df_investisseurs):
    region_flow = df_investisseurs.groupby('HQ Region')['2019-25 deals'].sum().sort_values(ascending=False)
    fig, ax = plt.subplots(figsize=(12, 6))
    sns.barplot(x=region_flow.values, y=region_flow.index, hue=region_flow.index, palette='magma', legend=False, ax=ax)
...
def plot_south_africa_investors(df_inv_final):
    ...
    bracket_colors = {'< $1M':'#95a5a6','$1M – $5M':'#3498db','$5M – $15M':'#27ae60',
                      '$15M – $50M':'#e67e22','$50M+':'#e74c3c'}
    bracket_counts = sa_stats['Ticket_Bracket'].value_counts()
    figs = []

    fig1, ax = plt.subplots(figsize=(12, 8))
    top20 = sa_stats.sort_values('Nb_Deals', ascending=False).head(20)
    bars  = ax.barh(top20['Investors_List'], top20['Nb_Deals'], color='#2c3e50', alpha=0.85)
    ...
    bars   = ax.barh(top20a['Investors_List'], top20a['Total_Deployed'], color='#e74c3c', alpha=0.85)
    ...
    for ax, bracket in zip(axes, brackets_to_plot):
        ...
        ax.axvline(m, color='gray', linestyle=':', linewidth=1.2, label=f'Median ({m:.0f})')
...
def plot_fundraising_window(df_deals):
    ...
    valid_transitions = [('Seed','Series A'),('Series A','Series B'),
                         ('Series B','Series C'),('Series C','Series D+')]
    transition_stats = {}
    for f, t in valid_transitions:
        mask = (df_gap['Prev_Stage']==f) & (df_gap['Type_Simple']==t) & (df_gap['Days_To_Next']>0)
        data = df_gap[mask]['Days_To_Next']
        if len(data) < 5: continue
        transition_stats[f] = {
            'to_stage': t, 'median_m': data.median()/30,
            'p25_m': data.quantile(0.25)/30, 'p75_m': data.quantile(0.75)/30,
            'danger_m': data.quantile(0.90)/30, 'n': len(data)
        }

    last_round = df_deals.sort_values('Deal Date').groupby('Start-up name').last().reset_index()
    last_round['Months_Since_Last'] = (pd.Timestamp('today') - last_round['Deal Date']).dt.days / 30
    ...
    in_win['Distance_To_Median'] = abs(in_win['Months_Since_Last'] - in_win['Median_Target'])
    top10 = in_win.sort_values('Distance_To_Median').head(10).reset_index(drop=True)

    stage_colors = {'Seed':'#3498db','Series A':'#2ecc71','Series B':'#e67e22','Series C':'#9b59b6'}
    ...
    axes[0].barh(list(y), [p75s[i]-p25s[i] for i in range(len(p25s))],
                 left=p25s, height=0.4, color='#27ae60', alpha=0.5, label='Normal window (P25–P75)')
    axes[0].scatter(medians, list(y), color='#27ae60', zorder=5, s=100, label='Median')
    axes[0].scatter(dangers, list(y), color='#e74c3c', zorder=5, s=100, marker='x', linewidths=2, label='Danger (P90)')
    ...
    for i, row in top10.iterrows():
        ...
        if pd.notna(row['Median_Target']):
            axes[1].vlines(row['Median_Target'], i-0.35, i+0.35, color='#27ae60', linestyle='--', linewidth=1.5)
        if pd.notna(row['Normal_Max']):
            axes[1].vlines(row['Normal_Max'], i-0.35, i+0.35, color='#e74c3c', linestyle='--', linewidth=1.5)
        axes[1].text(row['Months_Since_Last']+0.3, i,
                     f"→ {row['Next_Stage']}  ({row['Months_Since_Last']:.0f}m)", va='center', fontsize=9)

    axes[1].set_yticks(range(len(top10)))
    axes[1].set_yticklabels([f"{r['Start-up name']}  [{r['Type_Simple']}]" for _, r in top10.iterrows()], fontsize=10)
    axes[1].set_xlabel('Months since last round')
    axes[1].set_title('Top 10 Startups in Fundraising Window\n(Optimal timing to approach now)', fontsize=13)
    legend_e = [
...
def plot_knife_portfolio(df_inv_final):
    knife = df_inv_final[df_inv_final['Investors_List'].str.contains('Knife', case=False, na=False)].copy()
    if knife.empty:
        fig, ax = plt.subplots()
        ax.text(0.5, 0.5, 'No Knife Capital deals found', ha='center', va='center')
        return fig, None
    fig, axes = plt.subplots(2, 3, figsize=(20, 12))
    knife.groupby('Year').size().plot(kind='bar', ax=axes[0,0], color='#2c3e50', edgecolor='white')
    axes[0,0].set_title('Deals per Year')
    knife.groupby('Year')['Amount_clean'].sum().plot(kind='bar', ax=axes[0,1], color='#e74c3c', edgecolor='white')
    ...
    axes[1,1].hist(knife['Amount_clean'].dropna(), bins=10, color='#8e44ad', edgecolor='white')
...
def plot_knife_similar_deals(df_inv_final, df_deals):
    ...
    stage_order = {'Grants':0,'Pre-Seed':1,'Seed':2,'Series A':3,'Series B':4,'Series C':5,'Series D+':6,'Venture Round':2.5}
    ...
    candidates = df_inv_final[~df_inv_final['Start-up name'].isin(knife_startup_names)].copy()
    ...
    knife_vector     = np.array([[knife['Sector_Score'].mean(), knife['Country_Score'].mean(),
    ...
    scaler           = MinMaxScaler()
    all_scaled       = scaler.fit_transform(np.vstack([knife_vector, candidate_matrix]))
    ...
    startup_sim = candidates.groupby('Start-up name').agg(
        Sector=('Sector','last'), Country=('Country','last'),
        Last_Stage=('Type_Simple','last'), Total_Raised=('Amount_clean','sum'),
        Nb_Rounds=('Amount_clean','count'), Last_Round_Size=('Amount_clean','last'),
        Last_Round_Date=('Deal Date','max'), Is_YC=('is_yc','max'),
        Similarity_Score=('Similarity_Score','mean'),
    ).reset_index()
    startup_sim['Months_Since_Last'] = (pd.Timestamp('today') - startup_sim['Last_Round_Date']).dt.days / 30

    fundraising_window = {'Seed':(10,28),'Series A':(12,30),'Series B':(14,32),'Venture Round':(10,28)}
    def in_window(row):
    ...
    filtered = startup_sim[
        startup_sim['Last_Stage'].isin(['Seed','Series A','Series B','Venture Round']) &
        (startup_sim['Total_Raised'] < 200) & (startup_sim['Total_Raised'] > 0.5) &
    ...
    clean[momentum_cols] = clean[momentum_cols].replace([np.inf,-np.inf],np.nan).fillna(0)
    scaler2 = MinMaxScaler()
    clean[momentum_cols] = scaler2.fit_transform(clean[momentum_cols])
    clean['Momentum_Score'] = (clean['Total_Raised']*0.25 + clean['Nb_Rounds']*0.15 +
                                clean['Last_Round_Size']*0.25 + clean['Is_YC']*0.10 +
                                clean['Similarity_Score']*0.25)
    clean['Stage_Confidence'] = clean['Last_Stage'].map({'Seed':0.50,'Series A':0.65,'Series B':0.70,'Venture Round':0.55}).fillna(0.5)
    ...
    ax.set_title("Top 15 Deals Most Similar to Knife Capital's Investment DNA", fontsize=14)
...
def get_sa_data(df_inv_final):
    df_sa = df_inv_final[df_inv_final['Country'] == 'South Africa'].copy()
    sa_stats = (df_sa.groupby('Investors_List')
                .agg(Nb_Deals=('Start-up name','count'), Total_Deployed=('Amount_clean','sum'),
                     Median_Ticket=('Amount_clean','median'), Avg_Ticket=('Amount_clean','mean'),
                     Max_Ticket=('Amount_clean','max'), First_Deal=('Deal Date','min'),
                     Last_Deal=('Deal Date','max'))
                .reset_index()
                .rename(columns={'Investors_List':'Investor'}))
    sa_stats['Years_Active'] = ((sa_stats['Last_Deal'] - sa_stats['First_Deal']).dt.days / 365).round(1)
    def assign_bracket(m):
        if pd.isna(m) or m == 0: return 'Undisclosed'
        elif m < 1:   return '< $1M'
        elif m < 5:   return '$1M–$5M'
...
def plot_sa_overview(df_sa, sa_stats, df_inv_final):
    ...
    inv_by_year = (df_sa.groupby('Year')['Investors_List'].nunique().reset_index()
                   .rename(columns={'Investors_List':'Nb_Investors'}))
    axes[0,0].bar(inv_by_year['Year'], inv_by_year['Nb_Investors'], color='#2c3e50', edgecolor='white')
    axes[0,0].set_title('Active Investors per Year — SA', fontsize=12)
    axes[0,0].set_ylabel('Number of unique investors')
    axes[0,0].grid(axis='y', alpha=0.3)

    capital_year = df_sa.groupby('Year')['Amount_clean'].sum()
    axes[0,1].bar(capital_year.index, capital_year.values, color='#e74c3c', edgecolor='white')
    axes[0,1].set_title('Total Capital Deployed per Year ($M) — SA', fontsize=12)
    axes[0,1].grid(axis='y', alpha=0.3)

    if 'HQ Region' in df_inv_final.columns:
        df_sa_hq = df_sa.merge(df_inv_final[['Investors_List','HQ Region']].drop_duplicates(),
                                on='Investors_List', how='left')
        df_sa_hq['Origin'] = df_sa_hq['HQ Region'].apply(
            lambda x: 'Local (SA)' if str(x).strip() == 'Southern Africa' else 'International'
        )
        origin_counts = df_sa_hq['Origin'].value_counts()
        axes[0,2].pie(origin_counts.values, labels=origin_counts.index,
                      autopct='%1.1f%%', colors=['#27ae60','#3498db'], startangle=140)
        axes[0,2].set_title('Local vs International Investors — SA', fontsize=12)
    else:
        axes[0,2].axis('off')

    valid_tickets = sa_stats[sa_stats['Median_Ticket'].notna() & (sa_stats['Median_Ticket'] > 0)]['Median_Ticket']
    ...
    axes[1,1].plot(syndication_rate.index, syndication_rate.values, marker='o', color='#e67e22', linewidth=3)
    axes[1,1].set_title('Co-Investment Rate per Year (%) — SA', fontsize=12)
    axes[1,1].grid(alpha=0.3)
    axes[1,1].fill_between(syndication_rate.index, syndication_rate.values, alpha=0.15, color='#e67e22')

    axes[1,2].axis('off')
    stats_text = (
        f"Total active investors : {sa_stats['Investor'].nunique():,}\n"
        f"Total capital deployed : ${df_sa['Amount_clean'].sum():,.0f}M\n"
        f"Ecosystem median ticket : ${sa_stats['Median_Ticket'].median():.1f}M\n"
        f"Avg deals per investor : {sa_stats['Nb_Deals'].mean():.1f}\n"
        f"Avg co-investment rate : {deals_sa['Is_Syndicated'].mean()*100:.0f}%\n"
        f"Period covered : {df_sa['Year'].min()}–{df_sa['Year'].max()}"
    )
    axes[1,2].text(0.1, 0.5, stats_text, transform=axes[1,2].transAxes, fontsize=13,
                   verticalalignment='center', bbox=dict(boxstyle='round', facecolor='#f0f4ff', alpha=0.8))
    axes[1,2].set_title('SA Ecosystem Summary', fontsize=12)
    plt.suptitle('South Africa — Investor Ecosystem Overview', fontsize=18, fontweight='bold')
    plt.tight_layout()
    return fig
...
def plot_sa_sectors(df_sa):
    ...
    sns.barplot(x=top_s_vol.values, y=top_s_vol.index, hue=top_s_vol.index, palette='mako', legend=False, ax=axes[0,0])
    ...
    sns.barplot(x=top_s_cap.values, y=top_s_cap.index, hue=top_s_cap.index, palette='rocket', legend=False, ax=axes[0,1])
    ...
    axes[1,0].set_title('Sector Share Evolution — SA', fontsize=13)
    ...
    pivot = df_heat.groupby(['Investors_List','Sector']).size().unstack(fill_value=0)
    sns.heatmap(pivot, cmap='YlOrRd', annot=True, fmt='d', linewidths=0.5, ax=axes[1,1])
    axes[1,1].set_title('Investor × Sector Matrix (Top 15) — SA', fontsize=13)
    axes[1,1].tick_params(axis='x', rotation=45)
...
def plot_sa_tickets(df_sa, sa_stats):
    ...
    axes[0,0].plot(tick_year.index, tick_year.values, marker='o', color='#2c3e50', linewidth=3)
    ...
    tick_sector = df_sa.groupby('Sector')['Amount_clean'].median().sort_values(ascending=False).head(10)
    sns.barplot(x=tick_sector.values, y=tick_sector.index, hue=tick_sector.index, palette='viridis', legend=False, ax=axes[0,1])
    ...
    tick_stage = (df_sa[df_sa['Type_Simple'].isin(stage_order)]
                  .groupby('Type_Simple')['Amount_clean'].median().reindex(stage_order).dropna())
    axes[1,0].bar(tick_stage.index, tick_stage.values, color='#e67e22', edgecolor='white')
    ...
    axes[1,1].text(0.5, 0.5,
                   ...
                   f"Total investors: {sa_stats['Investor'].nunique()}\n"
...
def get_investor_profile(investor_name, df_sa, sa_stats):
    ...
    if inv_deals.empty: return None, None
...
def plot_investor_profile(investor_name, df_sa, sa_stats):
    ...
    if inv_deals is None:
        fig, ax = plt.subplots()
        ax.text(0.5, 0.5, f'No data found for {investor_name}', ha='center', va='center')
        return fig
    ...
    axes[0,1].barh(sector_dist.index, sector_dist.values, color=sns.color_palette('mako', len(sector_dist)))
    axes[0,1].set_title(f'Sector Distribution — {investor_name}', fontsize=12)
    axes[0,1].invert_yaxis()

    stage_dist = inv_deals['Type_Simple'].value_counts()
    axes[0,2].bar(stage_dist.index, stage_dist.values, color=sns.color_palette('Set2', len(stage_dist)))
    ...
    if not tick_ev.empty and tick_ev.notna().sum() > 0:
        axes[1,0].plot(tick_ev.index, tick_ev.values, marker='o', color='#e74c3c', linewidth=3)
    ...
    if not co_inv.empty:
        axes[1,1].barh(co_inv.index, co_inv.values, color='#8e44ad', edgecolor='white')
    ...
    if not inv_row.empty:
        ...
        axes[1,2].text(0.05, 0.5, stats_text, transform=axes[1,2].transAxes, fontsize=12,
                       verticalalignment='center', bbox=dict(boxstyle='round', facecolor='#f0f4ff', alpha=0.8))
    axes[1,2].set_title(f'Key Stats — {investor_name}', fontsize=12)
    plt.suptitle(f'Investor Profile: {investor_name}', fontsize=18, fontweight='bold')
    plt.tight_layout()
    return fig
...
def get_investor_portfolio_table(investor_name, df_sa):
    inv_deals = df_sa[df_sa['Investors_List'] == investor_name].copy()
    if inv_deals.empty: return pd.DataFrame()
    return (inv_deals[['Start-up name','Sector','Country','Year','Type_Simple','Amount_clean','Deal Date']]
            .rename(columns={'Type_Simple':'Stage','Amount_clean':'Amount ($M)'})
            .sort_values('Deal Date', ascending=False).reset_index(drop=True))
...
def build_coinvestment_network(df_sa, min_shared=2):
    ...
    edges_to_remove = [(u,v) for u,v,d in G.edges(data=True) if d['weight'] < min_shared]
...
def plot_coinvestment_network(df_sa, min_shared=2):
    ...
    if len(G.nodes()) == 0:
        fig, ax = plt.subplots()
        ax.text(0.5, 0.5, f'No co-investments with {min_shared}+ shared deals', ha='center', va='center')
        return fig
    ...
    nx.draw_networkx_edges(G, pos, ax=ax, width=edge_widths, alpha=0.4, edge_color='#7f8c8d')
    nx.draw_networkx_nodes(G, pos, ax=ax, node_size=node_sizes, node_color=node_colors, alpha=0.85)
    important_nodes = {n: n for n in G.nodes() if G.nodes[n]['deals_count'] >= 3}
    nx.draw_networkx_labels(G, pos, labels=important_nodes, ax=ax, font_size=8, font_weight='bold')

    ax.set_title(
        f'Co-Investment Galaxy — South Africa\n'
        f'({len(G.nodes())} investors, {len(G.edges())} connections, min. {min_shared} shared deals)\n'
        f'Node size = deal count | Edge thickness = co-investment frequency | Color = community cluster',
        fontsize=13
    )
    ax.axis('off')
    legend_elements = [mpatches.Patch(facecolor=colors[i], label=f'Cluster {i+1} ({len(c)} investors)')
...
def plot_sa_temporal(df_sa):
    ...
    pivot = (df_sa[df_sa['Investors_List'].isin(top20)]
             .groupby(['Investors_List','Year']).size().unstack(fill_value=0))
    sns.heatmap(pivot, cmap='Blues', annot=True, fmt='d', linewidths=0.5, ax=axes[0,0])
    ...
    df_sa2 = df_sa.copy()
    df_sa2['Month'] = pd.to_datetime(df_sa2['Deal Date']).dt.month
    monthly = df_sa2.groupby('Month').size()
    month_names = ['Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec']
    axes[1,0].bar(range(1,13), [monthly.get(m,0) for m in range(1,13)], color='#9b59b6', edgecolor='white')
    ...
    if not dormant.empty:
        ...
        axes[1,1].set_title('Last Active Year of Dormant Investors (2+ years inactive)', fontsize=12)
        axes[1,1].grid(axis='y', alpha=0.3)
    else:
        axes[1,1].axis('off')
        axes[1,1].text(0.5, 0.5, 'No dormant investors found', ha='center', va='center')

    plt.suptitle('South Africa — Investor Temporal Dynamics', fontsize=18, fontweight='bold')
    plt.tight_layout()
    return fig
...
def plot_sa_syndicates(df_sa):
    ...
    deal_groups = df_sa.groupby(['Start-up name','Year'])['Investors_List'].apply(list)
    pair_counts = {}; trio_counts = {}
    solo_investors = set(); syndicated_investors = set()
    ...
    for (startup, year), investors in deal_groups.items():
        ...
        if len(investors) == 1:
            ...
            for inv in investors: syndicated_investors.add(inv)
            for i in range(len(investors)):
                for j in range(i+1, len(investors)):
    ...
    if top_pairs:
        ...
        axes[0].barh(range(len(pair_labels)), [v for _,v in top_pairs], color='#3498db')
    ...
    if top_trios:
        ...
        axes[1].barh(range(len(trio_labels)), [v for _,v in top_trios], color='#27ae60')
    ...
    plt.suptitle('South Africa — Syndication & Co-Investment Patterns', fontsize=18, fontweight='bold')
    plt.tight_layout()
    return fig


def plot_sa_positioning_matrix(df_sa, sa_stats, highlight_investor=None):
    df_plot = sa_stats[sa_stats['Median_Ticket'].notna() &
                       (sa_stats['Median_Ticket'] > 0) &
                       (sa_stats['Nb_Deals'] >= 1)].copy()
    fig, ax = plt.subplots(figsize=(16, 10))
    max_cap = df_plot['Total_Deployed'].max()
    sizes   = ((df_plot['Total_Deployed'] / max_cap) * 2000 + 50).fillna(50)
    scatter = ax.scatter(df_plot['Median_Ticket'], df_plot['Nb_Deals'],
                         s=sizes, alpha=0.6, c=df_plot['Total_Deployed'],
                         cmap='viridis', edgecolors='white', linewidths=0.5)
    plt.colorbar(scatter, ax=ax, label='Total Capital Deployed ($M)')

    to_label = set(df_plot.nlargest(10,'Nb_Deals')['Investor']) | set(df_plot.nlargest(8,'Median_Ticket')['Investor'])
    for _, row in df_plot[df_plot['Investor'].isin(to_label)].iterrows():
        ax.annotate(row['Investor'][:20], (row['Median_Ticket'], row['Nb_Deals']),
    ...
    if highlight_investor:
        ...
        if not hi.empty:
            ax.scatter(hi['Median_Ticket'], hi['Nb_Deals'], s=500, color='#e74c3c',
                       zorder=5, edgecolors='black', linewidths=2)
            ax.annotate(f"◀ {highlight_investor}", (hi['Median_Ticket'].iloc[0], hi['Nb_Deals'].iloc[0]),
                        xytext=(10,0), textcoords='offset points', fontsize=10, color='#e74c3c', fontweight='bold')
    ...
    ax.set_title('Investor Positioning Matrix — South Africa\n(Bubble size = total capital deployed)', fontsize=14)
    ax.grid(alpha=0.2)
    plt.tight_layout()
    return fig        '$1M-$2M': 1.5, '$2M-$5M': 3.5, '$5M-$10M': 7.5,
        '$10M-$50M': 30.0, '$50M-$100M': 75.0, '$100M+': 150.0, 'Unknown': np.nan,
    }
    df_deals['Amount_clean'] = df_deals['Amount_numeric'].fillna(
        df_deals['Bracket'].map(bracket_map)
...
def plot_sa_positioning_matrix(df_sa, sa_stats, highlight_investor=None):
    ...
    ax.grid(alpha=0.2)
    plt.tight_layout()
    return fig        if val in ['', 'nan']: return np.nan
        if any(x in val for x in ['undisclosed', 'unknown', 'n/a']): return np.nan
